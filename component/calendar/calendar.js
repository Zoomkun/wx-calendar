...
Component({
 ...
 /**
     ...
     */
    attached(){
        const spotMap = this.data.spotMap
        let processSpotMap = {}
        if(spotMap.length > 0) {
            for (const iterator of spotMap) {
                if(iterator.status === 1) {
    ...
    methods: {
        swiperChange(e) { // 日历滑动时触发的方法
            ...
            if (dif === -2 || (dif > 0 && dif !== 2)) { //向右划的情况，日期增加
                if (this.data.open) {
                    ...
                    if((date.getMonth() + 1) > this.data.selectDay.month) {
                        this.triggerEvent("monthChange", this.data.selectDay)
                    }
        ...
        setSwiperHeight(index) { // 根据指定位置数组的大小计算长度
            this.setData({
                swiperHeight: this.data[`dateList${index}`].length / 7 * 64 + 32
            })
        },
        //更新指定的索引和月份的列表
        getIndexList({
